...
use macaco_sequential::{
    chen_et_al::{robust_matroid_center, VecWeightMap, WeightMap},
    kcenter::kcenter,
};
use std::rc::Rc;
use std::sync::Arc;
...
pub struct MapReduceCoresetRec<V> {
    tau: usize,
    coreset: Option<Vec<V>>,
    profile: Option<(Duration, Duration)>,
    counters: Option<(u64, u64)>,
...
impl<V: Distance + Clone + Weight + PartialEq> Algorithm<V> for MapReduceCoresetRec<V> {
    fn version(&self) -> u32 {
        1
    }

    fn name(&self) -> String {
        String::from("MRCoresetRec")
    }
    ...
    for MapReduceCoresetRec<T>
{
    fn parallel_run(
        ...
        let weights_par = VecWeightMap::new(coreset.iter().map(|p| p.1).collect());
        let coreset_par: Vec<T> = coreset.into_iter().map(|p| p.0).collect();

        // now compute the coreset of the coreset, summing the weights coming from the parallel coreset
        println!("Compute the coreset of the coreset");
        let (coreset, weights) = seq(self.tau, &coreset_par, weights_par, Rc::clone(&matroid));
        println!(
            "Size was {}, and now is {}",
            coreset_par.len(),
            coreset.len()
...
fn seq<V: Distance + Clone>(
    tau: usize,
    dataset: &[V],
    weights_orig: VecWeightMap,
    matroid: Rc<dyn Matroid<V>>,
) -> (Vec<V>, VecWeightMap) {
    let (centers, assignments) = kcenter(dataset, tau);

    // Build disks by assigning each center to the closest point
    let mut disks = vec![Vec::new(); centers.len()];
    for (point_idx, (v, i, _)) in assignments.enumerate() {
        disks[i].push((v, weights_orig.weight_of(point_idx)));
    }
    ...
    let coreset: Vec<(V, u32)> = disks
        .iter()
        .flat_map(|pairs| {
            let mut disk = Vec::with_capacity(pairs.len());
            for (v, _) in pairs {
                disk.push(*v);
            }
            assert!(disk.len() > 0);
    ...
    let weights = VecWeightMap::new(coreset.iter().map(|p| p.1).collect());
    (coreset.into_iter().map(|p| p.0).collect(), weights)
}
